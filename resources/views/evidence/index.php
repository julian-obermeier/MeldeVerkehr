...
<div class="grid"><?php foreach($items as $item):?><article class="item <?= $item['status']==='REMOVED'?'removed':'' ?>"><span class="tag"><?= $e($labels[$item['category']]??$item['category']) ?></span><h3><?= $e($item['original_filename']) ?></h3><p><strong><?= $e($quality[$item['quality_state']]??$item['quality_state']) ?></strong></p><p class="muted"><?= $e($item['width']) ?> × <?= $e($item['height']) ?> px · <?= $e(round(((int)$item['file_size'])/1024,1)) ?> KB · <?= $e($item['mime_type']) ?></p><p class="hash">SHA‑256: <?= $e($item['sha256']) ?></p><p class="muted">Status: <?= $e($item['status']) ?> · gespeichert: <?= $e($item['created_at']) ?></p>
