...
<form method="post" action="/evidence/<?= rawurlencode($item['id']) ?>/privacy/regions" id="regionForm"><input type="hidden" name="_csrf" value="<?= $e($csrf) ?>">
...
<button type="submit">Privacy-Bereich speichern</button></form></section>
...
<form method="post" action="/privacy-regions/<?= rawurlencode($region['id']) ?>/dismiss"><input type="hidden" name="_csrf" value="<?= $e($csrf) ?>"><input type="hidden" name="evidence_id" value="<?= $e($item['id']) ?>"><button class="danger" type="submit">Bereich entfernen</button></form></div><?php endforeach;?></div></section>
